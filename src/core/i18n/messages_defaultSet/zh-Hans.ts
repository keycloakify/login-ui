...
const messages = {
    ...
    bypassKerberosDetail:
        "您没有通过 Kerberos 登录，或者您的浏览器没有设置 Kerberos 登录。请点击继续通过其他途径登录",
    ...
    oauth2DeviceAuthorizationGrantDisabledMessage:
        "不允许此客户端启动 OAuth 2.0 设备授权授予。该流程已对此客户端禁用。",
    ...
    federatedIdentityUnavailableMessage:
        "使用身份提供者 {1} 进行身份验证的用户 {0} 不存在。请联系管理员。",
    federatedIdentityUnmatchedEssentialClaimMessage:
        "身份提供者签发的 ID 令牌与配置的基本要求不匹配。请联系管理员。",
    ...
    federatedIdentityConfirmOverrideMessage:
        "您正在尝试将您的帐号 {0} 与 {1} 帐号 {2} 连结，但您的帐号已经与不同的 {3} 帐号 {4} 连结。您确定要覆盖现有的连结吗？",
    ...
    identityProviderNotUniqueMessage:
        "该域支持多个身份提供程序。无法确定应使用哪个身份提供程序进行身份验证。",
    ...
    staleEmailVerificationLink:
        "您点击的链接是旧的过期链接，已经不再有效。 也许您已经验证了您的电子邮件。",
    ...
    confirmAccountLinkingBody:
        "如果您连结帐号，您将也能使用身分提供者 {1} 的帐号 {0} 来登入。如果这不是您所发起的操作，或您不希望连结帐号，请不要继续操作。",
    ...
    "console-verify-email":
        "您需要验证您的电子邮件地址。我们已向 {0} 发送了一封包含验证码的电子邮件。请将此代码输入下面的输入框中。",
    ...
    "recovery-authn-codes-display-name": "恢复代码",
    "recovery-authn-codes-help-text": "输入先前生成的恢复代码。",
    ...
    "recovery-code-config-warning-message":
        "确保打印、下载或将它们复制到密码管理器，并将其保存在安全的地方。取消此设置将从您的帐户中删除这些恢复代码。",
    ...
    "recovery-codes-download-file-description":
        "恢复代码是一次性验证码，如果您无法访问您的验证器，可以使用它们登录到您的帐户。",
    ...
    "error-invalid-multivalued-size":
        "属性 {0} 的数量必须至少有 {1} 个，最多 {2} 个{2,choice,0#值|1#值|1<值}。",
    ...
    emailVerificationPending:
        "一封验证用的电子信件已寄送到 {0}。您可以直接提交并重新发送验证电子邮件，或输入其他电子邮件地址。",

...
const messages = {
    ...
    loginTotpIntro:
        "このアカウントにアクセスするには、ワンタイムパスワードジェネレーターを設定する必要があります",
    loginTotpStep1: "次のいずれかのアプリケーションをモバイルにインストールします。",
    loginTotpStep2: "アプリケーションを開き、バーコードをスキャンします。",
    loginTotpStep3:
        "アプリケーションから提供されたワンタイムコードを入力し、「送信」をクリックしてセットアップを終了します。",
    ...
    backToLogin: "« ログインに戻る",
    emailInstruction:
        "ユーザー名またメールアドレスを入力してください。新しいパスワードの設定方法をお知らせします。",
    ...
    delegationCompleteMessage:
        "このブラウザーのウィンドウを閉じて、コンソールアプリケーションに戻ることができます。",
    delegationFailedHeader: "ログインに失敗しました",
    delegationFailedMessage:
        "このブラウザーウィンドウを閉じてコンソールアプリケーションに戻り、再度ログインを試みることができます。",
    ...
    invalidPasswordHistoryMessage:
        "無効なパスワード: 直近{0}個のパスワードのいずれかと同じパスワードは禁止されています。",
    ...
    bearerOnlyMessage:
        "bearer-onlyのアプリケーションはブラウザーログインを開始することが許可されていません",
    standardFlowDisabledMessage:
        "与えられたresponse_typeでクライアントはブラウザーログインを開始することが許可されていません。標準フローは無効です。",
    implicitFlowDisabledMessage:
        "与えられたresponse_typeでクライアントはブラウザーログインを開始することが許可されていません。インプリシットフローは無効です。",
    ...
    identityProviderUnexpectedErrorMessage:
        "アイデンティティープロバイダーによる認証の際に予期せぬエラーが発生しました",
    identityProviderNotFoundMessage:
        "該当の識別子を持つアイデンティティープロバイダーが見つかりませんでした。",
    identityProviderLinkSuccess:
        "メールを正常に検証しました。元のブラウザーに戻ってログインしてください。",
    staleCodeMessage:
        "このページはもはや有効ではありませんので、アプリケーションに戻り再度サインインしてください",
    ...
    identityProviderNotUniqueMessage:
        "レルムは複数のアイデンティティープロバイダーをサポートしています。どのアイデンティティープロバイダーが認証に使用されるべきか判断できませんでした。",
    ...
    confirmAccountLinking:
        "アイデンティティープロバイダー{1}のアカウント{0}とあなたのアカウントとのリンクを確認してください。",
    ...
    brokerLinkingSessionExpired:
        "要求されたブローカーアカウントのリンクは、現在のセッションでは有効ではありません。",
    ...
    "webauthn-error-different-user":
        "最初に認証されたユーザーは、パスキーによって認証されたユーザーではありません。",
    ...
    deletingAccountForbidden:
        "アカウントを削除するために必要な権限がありません。管理者に連絡してください。",
    ...
    emailUpdateConfirmationSent:
        "確認メールが{0}に送信されました。指示に従い、メールアドレスの更新を完了してください。",
    usb: "USB",
    internal: "内部",
    ...
    oauth2DeviceVerificationCompleteMessage:
        "このブラウザーのウィンドウを閉じて、デバイスに戻ることができます。",
    ...
    "passkey-unsupported-browser-text":
        "このブラウザーではパスキーがサポートされていません。別のブラウザーを試すか、管理者にお問い合わせください。",
    ...
    invalidPasswordNotEmailMessage:
        "無効なパスワード: メールアドレスと同じパスワードは禁止されています。",
    ...
    "webauthn-unsupported-browser-text":
        "このブラウザーではWebAuthnはサポートされていません。別のブラウザーを試すか、管理者にお問い合わせください。",
    ...
    emailVerifyInstruction4:
        "メールアドレスを確認するため、これから{0}のアドレスに指示のメールを送ります。",
    ...
    federatedIdentityUnavailableMessage:
        "アイデンティティープロバイダー{1}で認証されたユーザー{0}が存在しません。管理者に連絡してください。",
    confirmOverrideIdpTitle: "ブローカーのリンクがすでに存在します",
    federatedIdentityConfirmOverrideMessage:
        "アカウント{0}を{1}のアカウント{2}にリンクしようとしています。しかし、アカウントは既に別の{3}のアカウント{4}とリンクされています。既存のリンクを新しいアカウントに置き換えますか？",
    ...
    identityProviderMissingStateMessage:
        "アイデンティティープロバイダーからのレスポンスにstateパラメーターがありません。",
    identityProviderInvalidSignatureMessage:
        "アイデンティティープロバイダーからのレスポンスの署名が無効です。",
    "requiredAction.CONFIGURE_RECOVERY_AUTHN_CODES": "リカバリーコードの生成",
    "requiredAction.webauthn-register-passwordless": "パスワードレスWebAuthnの登録",
    "requiredAction.webauthn-register": "WebAuthnの登録",
    ...
    "recovery-authn-codes-help-text": "以前に生成されたリストからリカバリー認証コードを入力します。",
    ...
    "error-invalid-multivalued-size":
        "属性{0}は少なくとも{1}、多くても{2} {2,choice,0#値|1#値|1<値}でなければならない。",
    ...
    "recovery-code-config-warning-title":
        "これらのリカバリーコードは、このページを離れるともう表示されません",
    emailInstructionUsername:
        "ユーザー名を入力してください。新しいパスワードの作成方法をお知らせします。",
    ...
    "recovery-code-config-warning-message":
        "必ず印刷、ダウンロード、またはパスワードマネージャーにコピーして保管してください。この設定をキャンセルすると、これらのリカバリーコードはアカウントから削除されます。",
    "recovery-codes-download-file-description":
        "リカバリーコードは、オーセンティケーターにアクセスできない場合にアカウントにサインインするための 1 回限りのパスコードです。",
    federatedIdentityUnmatchedEssentialClaimMessage:
        "アイデンティティープロバイダーから発行されたIDトークンが、設定された必須クレームと一致しません。管理者に連絡してください。",
    cookieNotFoundMessage:
        "再起動ログインCookieが見つかりません。有効期限が切れているか、削除されているか、ブラウザーでCookieが無効になっている可能性があります。Cookieが無効になっている場合は、有効にしてください。「アプリケーションに戻る」をクリックして再度ログインしてください。",
    linkIdpActionTitle: "{0} をリンク",
    linkIdpActionMessage: "アカウントを{0}にリンクしますか？",
    confirmAccountLinkingBody:
        "アカウントをリンクすると、IDプロバイダー{1}のアカウント{0}を使用してログインできるようになります。このプロセスを開始していない場合、またはアカウントをリンクしたくない場合は、続行しないでください。",
    emailVerifySendCooldown: "検証メールを再送する前に{0}秒待つ必要があります。",
    "error-non-ascii-local-part-email": "住所の地域部分はASCII文字のみを含める必要があります。",
    "requiredAction.delete_account": "アカウントの削除",
    "requiredAction.UPDATE_EMAIL": "Eメールの更新",
    "requiredAction.VERIFY_PROFILE": "プロファイルの検証",
    ...
    emailVerificationPending:
        "確認メールが{0}に送信されました。変更せずに送信して確認メールを再送信するか、別のメールアドレスを入力してください。",
    orgMemberAlready: "すでに{1}の組織のメンバーです。",
    staleInviteOrgLink:
        "クリックしたリンクは無効です。期限が切れているか、既に使用されている可能性があります。",

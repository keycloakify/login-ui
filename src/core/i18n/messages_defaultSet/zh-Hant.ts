...
const messages = {
    ...
    bypassKerberosDetail:
        "您沒有通過 Kerberos 登入 或者您的瀏覽器沒有設定 Kerberos 登入。請點擊繼續使用其他方式登入",
    ...
    expiredCodeMessage: "登入超時，請重新登入。",
    expiredActionMessage: "動作超時，請繼續登入。",
    expiredActionTokenNoSessionMessage: "動作超時。",
    ...
    federatedIdentityUnavailableMessage:
        "在 {1} 身分提供者上驗證的使用者 {0} 不存在。請聯繫您的管理員。",
    federatedIdentityUnmatchedEssentialClaimMessage:
        "身分提供者所簽發的 ID 權杖與設定的聲明不符。請聯絡您的管理員。",
    ...
    federatedIdentityConfirmOverrideMessage:
        "您正在嘗試將您的帳號 {0} 與 {1} 帳號 {2} 連結，但您的帳號已經與不同的 {3} 帳號 {4} 連結。您確定要覆蓋現有的連結嗎？",
    ...
    updateEmailMessage: "您需要更新您的電子郵件地址來啟用您的帳號。",
    resetPasswordMessage: "您需要更新您的密碼。",
    verifyEmailMessage: "您需要驗證您的電子郵件地址來啟用您的帳號。",
    linkIdpMessage: "您需要驗證您的電子郵件地址來將您的帳號與 {0} 連結。",
    ...
    standardFlowDisabledMessage:
        "應用程式不允許使用給定的 response_type 啟動瀏覽器登入。標準流程已被該應用程式禁用。",
    implicitFlowDisabledMessage:
        "應用程式不允許使用給定的 response_type 啟動瀏覽器登入。隱含流程已被該應用程式禁用。",
    ...
    cookieNotFoundMessage:
        "Cookie 不存在！Cookie 可能已過期、刪除、或者您的瀏覽器不允許使用 Cookie。請檢查 Cookie 設定並確保已啟用 Cookie。點擊上一頁並重新登入應用程式。",
    ...
    identityProviderNotUniqueMessage:
        "領域支援多個身份提供者。無法決定應該使用哪個身份提供者進行身份驗證。",
    emailVerifiedMessage: "您的電子郵件地址已通過驗證。",
    emailVerifiedAlreadyMessage: "您的電子郵件地址已經完成驗證。",
    ...
    confirmAccountLinkingBody:
        "如果您連結帳號，您將也能使用身分提供者 {1} 的帳號 {0} 來登入。如果這不是您所發起的操作，或您不希望連結帳號，請不要繼續操作。",
    ...
    "console-verify-email":
        "您需要驗證您的電子郵件。我們已經發送了一封包含驗證碼的電子郵件給 {0}。請將此碼輸入到下面的輸入框中。",
    ...
    "recovery-authn-codes-display-name": "恢復驗證碼",
    "recovery-authn-codes-help-text": "輸入先前產生的清單中的恢復驗證碼。",
    ...
    "recovery-code-config-warning-message":
        "請確保列印、下載或複製到密碼管理器中，並將其保存。取消此設定將從您的帳戶中刪除這些恢復驗證碼。",
    ...
    "recovery-codes-download-file-description":
        "恢復驗證碼是一次性密碼，如果您無法存取您的驗證器，將允許您透過恢復驗證碼登入您的帳戶。",
    ...
    "webauthn-unsupported-browser-text":
        "此瀏覽器不支援 WebAuthn。請嘗試其他瀏覽器或聯絡您的系統管理員。",
    ...
    "identity-provider-login-label": "或透過其他登入方式",
    ...
    "error-invalid-multivalued-size":
        "屬性 {0} 的數量必須至少有 {1} 個，最多 {2} 個{2,choice,0#值|1#值|1<值}。",
    ...
    "requiredAction.delete_account": "刪除帳號",
    "requiredAction.UPDATE_EMAIL": "更新電子信箱",
    "requiredAction.VERIFY_PROFILE": "驗證身分檔",
    ...
    emailVerificationPending:
        "一封驗證用的電子信件已寄送到 {0}。您可以直接提交並重新發送驗證電子郵件，或輸入其他電子郵件地址。",
    emailVerificationCancelled: "管理員已取消這個電子信箱驗證。",
    orgMemberAlready: "您已經是 {1} 組織的成員了。",
    staleInviteOrgLink: "您所點擊的連結已失效。該連結可能已經過期或已被使用。",

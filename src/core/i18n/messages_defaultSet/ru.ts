...
const messages = {
    ...
    deletingAccountForbidden:
        "У вас недостаточно прав для удаления учетной записи, обратитесь к администратору.",
    ...
    bypassKerberosDetail:
        "Либо вы не вошли в систему с помощью Kerberos, либо ваш браузер не настроен для входа в систему Kerberos. Пожалуйста, нажмите кнопку 'Продолжить' для входа с помощью других средств",
    ...
    oauthGrantInformation:
        "Убедитесь, что вы доверяете {0}, узнав, как {0} будет обрабатывать ваши данные.",
    ...
    emailUpdateConfirmationSent:
        "Письмо с подтверждением отправлено на {0}. Следуйте инструкциям, чтобы завершить обновление электронной почты.",
    ...
    loginTotpIntro:
        "Вам необходимо настроить генератор одноразовых паролей для доступа к этой учетной записи",
    ...
    loginTotpStep3:
        "Введите одноразовый код, выданный приложением, и нажмите Подтвердить для завершения настройки.",
    loginTotpStep3DeviceName:
        "Укажите имя устройства, которое поможет вам найти его в списке ваших устройств.",
    ...
    oauth2DeviceExpiredUserCodeMessage:
        "Код устарел. Пожалуйста, вернитесь к устройству и попробуйте подключиться снова.",
    oauth2DeviceVerificationCompleteHeader: "Успешный вход в устройство",
    oauth2DeviceVerificationCompleteMessage:
        "Вы можете закрыть это окно браузера и вернуться на свое устройство.",
    oauth2DeviceVerificationFailedHeader: "Ошибка входа в устройство",
    oauth2DeviceVerificationFailedMessage:
        "Вы можете закрыть это окно браузера, вернуться к своему устройству и повторить попытку подключения.",
    ...
    oauth2DeviceAuthorizationGrantDisabledMessage:
        "Клиенту не разрешено инициировать OAuth 2.0 Device Authorization Grant. Поток отключен для клиента.",
    emailVerifyInstruction1:
        "Вам было отправлено письмо с инструкциями для подтверждения адреса E-mail {0}.",
    ...
    emailVerifyInstruction4:
        "Для проверки вашего адреса электронной почты мы отправим вам письмо с инструкциями на адрес {0}.",
    ...
    emailInstructionUsername:
        "Введите свое имя пользователя, и мы вышлем вам инструкции по созданию нового пароля.",
    ...
    federatedIdentityUnavailableMessage:
        "Пользователь {0}, аутентифицированный с помощью поставщика удостоверений {1}, не существует. Обратитесь к администратору.",
    federatedIdentityUnmatchedEssentialClaimMessage:
        "Идентификационный токен, выданный поставщиком удостоверений, не соответствует настроенному essential claim. Обратитесь к администратору.",
    ...
    federatedIdentityConfirmOverrideMessage:
        "Вы пытаетесь связать свою учетную запись {0} с учетной записью {1} {2}. Но ваша учетная запись уже связана с другой {3} учетной записью {4}. Можете ли вы подтвердить, хотите ли вы заменить существующую ссылку на новую учетную запись?",
    ...
    configureBackupCodesMessage:
        "Для активации вашей учетной записи вам необходимо настроить резервные коды.",
    ...
    delegationCompleteMessage:
        "Вы можете закрыть это окно браузера и вернуться в консольное приложение.",
    delegationFailedHeader: "Ошибка входа",
    delegationFailedMessage:
        "Вы можете закрыть это окно браузера, вернуться в консольное приложение и попробовать войти снова.",
    ...
    cookieNotFoundMessage:
        "Перезапустить вход cookie не найден. Возможно, истек срок его действия; он был удален или в вашем браузере отключены файлы cookie. Если файлы cookie отключены, включите их. Нажмите Вернуться в приложение, чтобы снова войти.",
    ...
    identityProviderMissingStateMessage:
        "Отсутствует параметр состояния в ответе от поставщика удостоверений.",
    identityProviderMissingCodeOrErrorMessage:
        "Отсутствует код или параметр ошибки в ответе от поставщика удостоверений.",
    identityProviderInvalidResponseMessage: "Неверный ответ от поставщика удостоверений.",
    identityProviderInvalidSignatureMessage:
        "Недействительная подпись в ответе от поставщика удостоверений.",
    ...
    staleEmailVerificationLink:
        "Ссылка, по которой Вы перешли, устарела и больше не действует. Возможно, вы уже подтвердили свой E-mail.",
    identityProviderAlreadyLinkedMessage:
        "Федеративная идентификация, возвращенная {0}, уже связана с другим пользователем.",
    confirmAccountLinking:
        "Подтвердите привязку учетной записи {0} поставщика удостоверений {1} к вашей учетной записи.",
    ...
    differentUserAuthenticated:
        "Вы уже аутентифицированы как другой пользователь '{0}' в этой сессии. Пожалуйста, сначала выйдете.",
    brokerLinkingSessionExpired:
        "Запрошена привязка брокерского счета, но текущая сессия больше не действительна.",
    ...
    "console-verify-email":
        "Вам необходимо подтвердить свой адрес электронной почты. Мы отправили письмо на адрес {0}, содержащее код подтверждения. Введите этот код в поле ниже.",
    ...
    "saml.post-form.js-disabled":
        "JavaScript отключен. Настоятельно рекомендуем включить его. Нажмите кнопку ниже, чтобы продолжить.",
    ...
    "auth-x509-client-username-form-help-text":
        "Войдите в систему, используя клиентский сертификат X509.",
    "recovery-authn-codes-display-name": "Восстановление кода аутентификации",
    "recovery-authn-codes-help-text":
        "Введите код восстановления аутентификации из ранее сгенерированного списка.",
    ...
    "recovery-code-config-warning-title":
        "Эти коды восстановления больше не появятся после того, как вы покинете эту страницу",
    "recovery-code-config-warning-message":
        "Обязательно распечатайте, загрузите или скопируйте их в менеджер паролей и сохраните. Отмена этой настройки удалит эти коды восстановления из вашей учетной записи.",
    ...
    "recovery-codes-download-file-description":
        "Коды восстановления — это одноразовые коды доступа, которые позволяют вам войти в свою учетную запись, если у вас нет доступа к аутентификатору.",
    ...
    "webauthn-unsupported-browser-text":
        "WebAuthn не поддерживается этим браузером. Попробуйте другой или обратитесь к администратору.",
    ...
    "webauthn-registration-init-label-prompt":
        "Пожалуйста, введите метку вашего зарегистрированного ключа доступа",
    ...
    "webauthn-error-different-user":
        "Первый аутентифицированный пользователь не совпадает с пользователем, прошедшим аутентификацию через ключ доступа.",
    ...
    "passkey-unsupported-browser-text":
        "Passkey не поддерживается этим браузером. Попробуйте другой или обратитесь к администратору.",
    ...
    "idp-email-verification-help-text":
        "Привяжите свою учетную запись, подтвердив адрес электронной почты.",
    ...
    finalDeletionConfirmation:
        "Если вы удалите свой аккаунт, его нельзя будет восстановить. Чтобы сохранить свой аккаунт, нажмите «Отмена».",
    ...
    accountUnusable:
        "Любое последующее использование приложения с этой учетной записью будет невозможно",
    ...
    readOnlyUsernameMessage:
        "Вы не можете обновить свое имя пользователя, так как оно доступно только для чтения.",
    "error-invalid-multivalued-size":
        "Атрибут {0} должен иметь не менее {1} и не более {2} {2,choice,0#values|1#value|1<values}.",
    "organization.confirm-membership.title":
        "Вы собираетесь присоединиться к организации ${kc.org.name}",
    "organization.confirm-membership": "Нажав на ссылку ниже, вы станете членом организации {0}:",
    "organization.member.register.title":
        "Создайте учетную запись, чтобы присоединиться к организации ${kc.org.name}",
    ...
    confirmAccountLinkingBody:
        "Если вы привяжете учетную запись, вы также сможете войти, используя учетную запись {0} поставщика удостоверений {1}. Не продолжайте, если вы не инициировали этот процесс или не хотите привязывать учетную запись.",
    codeSuccess: "Код успеха",
    emailVerifySendCooldown:
        "Вам необходимо подождать {0} секунд перед повторной отправкой проверочного письма.",
    "error-non-ascii-local-part-email": "Локальная часть адреса должна содержать только символы ASCII.",
    emailVerifiedMessageHeader: "Адрес электронной почты подтвержден",
    emailVerifiedAlreadyMessageHeader: "Адрес электронной почты подтвержден",
    confirmEmailAddressVerificationHeader: "Проверка адреса электронной почты",
    "requiredAction.delete_account": "Удалить аккаунт",
    "requiredAction.UPDATE_EMAIL": "Обновить электронную почту",
    "requiredAction.VERIFY_PROFILE": "Верифицировать профиль",
    "requiredAction.delete_credential": "Удалить учетные данные",
    "requiredAction.idp_link": "Привязка провайдера идентификации",
    "requiredAction.update_user_locale": "Обновить локаль пользователя",
    "code-clipboard-label": "Показать содержимое",
    "code-copy-label": "Копировать в буфер обмена",
    "code-copy-success": "Код скопирован в буфер обмена",
    "code-copy-failure": "Не удалось скопировать код в буфер обмена",
    emailVerificationPending:
        "Письмо с подтверждением было отправлено на адрес {0}. Вы можете отправить его без изменений, чтобы повторно отправить письмо с подтверждением, или ввести другой адрес электронной почты.",
    ...
    staleInviteOrgLink:
        "Ссылка, по которой вы перешли, больше недействительна. Возможно, срок ее действия истек или она уже была использована.",

...
const messages = {
    ...
    bypassKerberosDetail:
        "Kerberos로 로그인되지 않았거나 브라우저가 Kerberos 로그인을 지원하지 않습니다. 다른 방법으로 로그인하려면 계속을 클릭하세요",
    ...
    emailUpdateConfirmationSent:
        "{0}으로 확인 이메일이 전송되었습니다. 이메일 업데이트를 완료하려면 전송된 지침을 따르세요.",
    ...
    loginTotpStep3:
        "애플리케이션에서 제공하는 일회용 코드를 입력하고 설정을 완료하려면 제출을 클릭하세요.",
    ...
    oauth2DeviceAuthorizationGrantDisabledMessage:
        "클라이언트는 OAuth 2.0 장치 권한 부여 흐름을 시작할 수 없습니다. 클라이언트에 대한 흐름이 비활성화되어 있습니다.",
    ...
    emailVerifyInstruction4:
        "이메일 주소를 확인하기 위해 {0} 주소로 지침이 담긴 이메일을 보낼 예정입니다.",
    ...
    emailInstruction:
        "사용자 이름 또는 이메일 주소를 입력하면 새 비밀번호를 생성하는 방법에 대한 지침을 보내드립니다.",
    emailInstructionUsername:
        "사용자 이름을 입력하면 새 비밀번호를 생성하는 방법에 대한 지침을 보내드립니다.",
    ...
    federatedIdentityExistsMessage:
        "{0} {1} 사용자와 일치하는 계정이 이미 존재합니다. 계정 관리를 위해 로그인하세요.",
    federatedIdentityUnavailableMessage:
        "{0} 사용자가 ID 공급자 {1}로 인증되지 않았습니다. 관리자에게 문의하세요.",
    ...
    federatedIdentityConfirmLinkMessage:
        "{0} {1} 사용자와 일치하는 계정이 이미 존재합니다. 계속하려면 어떻게 하시겠습니까?",
    federatedIdentityConfirmOverrideMessage:
        "계정 {0}을(를) {1} 계정 {2}와 연결하려고 하지만, 현재 계정은 다른 {3} 계정 {4}과 연결되어 있습니다. 기존 링크를 새 계정으로 교체하시겠습니까?",
    ...
    implicitFlowDisabledMessage:
        "클라이언트는 주어진 response_type으로 브라우저 로그인을 시작할 수 없습니다. 클라이언트에 대해 암시적 흐름이 비활성화되었습니다.",
    ...
    unexpectedErrorHandlingRequestMessage:
        "ID 공급자로 인증 요청을 처리하는 중에 예기치 않은 오류가 발생했습니다.",
    ...
    identityProviderLinkSuccess:
        "이메일을 성공적으로 확인했습니다. 원래 브라우저로 돌아가 로그인 절차를 계속하세요.",
    staleCodeMessage:
        "이 페이지는 더 이상 유효하지 않습니다. 애플리케이션으로 돌아가 다시 로그인하세요.",
    ...
    identityProviderNotUniqueMessage:
        "영역이 여러 ID 공급자를 지원합니다. 어떤 ID 공급자로 인증해야 할지 결정할 수 없습니다.",
    ...
    staleEmailVerificationLink:
        "클릭한 링크는 오래된 링크로 더 이상 유효하지 않습니다. 이미 이메일을 확인했을 수도 있습니다.",
    identityProviderAlreadyLinkedMessage:
        "{0}에서 반환된 연합 ID가 다른 사용자와 이미 연결되어 있습니다.",
    confirmAccountLinking: "ID 공급자 {1}의 계정 {0}을 계정에 연결하는 것을 확인하세요.",
    confirmAccountLinkingBody:
        "계정을 연결하면 ID 공급자 {1}의 계정 {0}을 사용하여 로그인할 수도 있습니다. 이 프로세스를 시작하지 않았거나 계정을 연결하고 싶지 않은 경우 계속하지 마십시오.",
    ...
    "console-verify-email":
        "이메일 주소를 확인해야 합니다. {0}으로 확인 코드가 포함된 이메일을 보냈습니다. 아래 입력란에 이 코드를 입력하세요.",
    ...
    "saml.post-form.js-disabled":
        "JavaScript가 비활성화되어 있습니다. 활성화할 것을 강력히 권장합니다. 계속하려면 아래 버튼을 클릭하세요. ",
    ...
    "recovery-authn-codes-help-text": "이전에 생성된 목록에서 복구 인증 코드를 입력하세요.",
    ...
    "recovery-code-config-warning-title":
        "이 페이지를 떠나면 이러한 복구 코드는 다시 표시되지 않습니다.",
    ...
    "recovery-codes-download-file-description":
        "복구 코드는 인증기에 접근할 수 없는 경우 계정에 로그인할 수 있는 일회용 암호입니다.",

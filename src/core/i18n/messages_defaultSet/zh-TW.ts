...
const messages = {
    ...
    "oauth2DeviceAuthorizationGrantDisabledMessage":
        "客戶端不允許發起授權 OAuth 2.0 裝置。這個流程已被客戶端禁用。",
    ...
    "federatedIdentityUnmatchedEssentialClaimMessage":
        "身分提供者所發出的 ID 令牌與設定的聲明不符。請聯絡您的管理員。",
    ...
    "standardFlowDisabledMessage":
        "客戶端不允許使用給定的 response_type 啟動瀏覽器登入。標準流程已被該客戶端禁用。",
    "implicitFlowDisabledMessage":
        "客戶端不允許使用給定的 response_type 啟動瀏覽器登入。隱含流程已被該客戶端禁用。",
